    ...
    for interval in ${intervals[@]}; do
        for script in ${scripts[@]}; do
            ...
            if [[ $script == "main.py" ]]; then
                data_dir="pth/crash_${interval}_origin_${seed}.pth"
                log_dir="log/crash_${interval}_origin_${seed}.txt"
            else
                data_dir="pth/crash_${interval}_nb_${seed}.pth"
                log_dir="log/crash_${interval}_nb_${seed}.txt"
            fi
            
            # Execute the script and launch the process in the background
            python $script --enable-cuda --using-data crash_$interval --data-dir $data_dir --log-dir $log_dir --seed $seed &
    ...
    for interval in ${intervals[@]}; do
        for script in ${scripts[@]}; do
            ...
            if [[ $script == "test.py" ]]; then
                data_dir="pth/crash_${interval}_origin_${seed}.pth"
                output_dir="output/crash_${interval}_origin_${seed}"
            else
                data_dir="pth/crash_${interval}_nb_${seed}.pth"
                output_dir="output/crash_${interval}_nb_${seed}"
            fi
            
            # Execute the script and launch the process in the background
            python $script --using-data crash_$interval --best-model $data_dir --output-dir $output_dir &
